...
struct InferAttrPassArgs {
  std::vector<AttrType> input_attrs;
  // The key name in the attribute dictionary of each node that the inference algorithm
  // could look for for extra inference hints.
  // For example:
  //   For shape inference, one could explicitly set "__shape__" in the node->attrs and
  //   pass "__shape__" as `node_attr_key` so that the inference algorithm will
  //   directly use the given shape.
